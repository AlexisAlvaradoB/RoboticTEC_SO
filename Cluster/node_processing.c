...
struct LetterList * processText(char * textToProcess){


    struct LetterList * dictionary = (struct LetterList *)malloc(sizeof(struct LetterList) * 26);
    ...
    int letterCount = 0;
    int especialSimbol = 0;//Para tildes, eñes, díerecisi
    long charProcessed = 0;
    while (true){
        fileText = textToProcess[charProcessed];
